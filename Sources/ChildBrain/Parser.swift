...
private typealias Stream = String
...
private func satisfy(_ condition: @escaping (Character) -> Bool) -> Parser<Character> {
    return { stream in
        ...
        return (firstCharacter, String(stream.dropFirst()))
    }
}
...
public func parse(_ input: String) -> (Value, String)? {
    ...
    guard let (result, remainder) = value(input) else { return nil }
...
func isNumber(_ input: String) -> Bool {
    if let (_, remainder) = number(input) {
...
public func map(of input: String) -> [String: String] {
    guard let (result, _) = pairs(input) else { return [:] }
...
public func list(of input: String) -> [(String, [(String, String?)]?)] {
    ...
    guard let (result, _) = _list(input) else { return [] }
    return result
}
